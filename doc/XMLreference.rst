...
:at:`onwarn`: :at-val:`[auto, continue, stop], "auto"`
   ...
   dropping and pivot clamping still apply. In :at-val:`stop` mode the top-level pipeline function stops at the first
   warning, leaving the state available for inspection. In all three modes the warning is counted in
   ``mjData.warning`` and reported in ``mjData.status``.
