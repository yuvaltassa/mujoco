...
.. collapse:: Usage example of frame
   ...
   the compiled orientation of Alice is ``quat="0 0 0 1"``, Bob is at ``pos="0 2 0"`` and Carl at ``pos="1 1 0"``.
   Saving the model writes the frames and the frame-relative values as above.
...
:at:`fusestatic`: :at-val:`[false, true], "false" for MJCF, "true" for URDF`
   ...
   - They are :ref:`mocap<body-mocap>` bodies.
...
transformation they encode is accumulated in their direct children; frames are preserved when the model is saved. See
:ref:`frame<frame>` for examples.
