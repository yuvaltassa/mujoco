...
manifested as an :ref:`mjWARN_BADQACC<mjtwarning>` :ref:`simulation warning<siSimWarning>`, reported in
``mjData.status`` by :ref:`mj_step` and handled according to the :ref:`onwarn<option-onwarn>` option. Divergence is
endemic to all physics simulation and is not necessarily indicative of a bad model or bug in the simulator, but is
rather a hint that the timestep is too large for
