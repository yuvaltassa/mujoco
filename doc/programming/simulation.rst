...
Simulation warnings are reported in three ways. First, every pipeline function records its status in ``mjData.status``,
of type :ref:`mjtStatus`: ``mjSTATUS_OK`` (0) means that no simulation warning was recorded, and a positive value names
the first warning raised during the call. The status belongs to the outermost pipeline call; the stages it runs, and
pipeline calls made from callbacks on the same ``mjData``, report into it. The macro :ref:`mjOK` tests the status.
Second, the array ``mjData.warning`` accumulates per-type :ref:`statistics<siDiagnostics>`. Third, when a warning of a
given type is first triggered, a warning-level :ref:`log message<siLogMessages>` is emitted. All this is done by the
function :ref:`mj_warning`, which the engine calls when it detects one. It is not an interface for user code, and is
deprecated as one: called from outside the engine it injects a warning into whatever pipeline call happens to be
running. To report a condition from a callback, emit a :ref:`log message<siLogMessages>` with :ref:`mju_warning`. To
treat a warning as an error, check the status after the call:
``mj_step(m, d); if (!mjOK(d)) mju_error(...);``.
