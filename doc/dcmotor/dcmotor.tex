...
\midrule
\texttt{input}      & keyword & Input block: subset of \texttt{pos vel ff voltage}, or \texttt{none} \\
...
\midrule
\texttt{input} & keyword & subset of \texttt{pos vel ff voltage}, in canonical order; \texttt{none} (passive) \\
...
\node[mode] at (0, 1.45) {$\begin{aligned}
  ...
  &\phantom{=}\ \text{absent inputs are zero;}\quad \texttt{input=\textquotedbl none\textquotedbl:}\ v = 0
\end{aligned}$};
...
velocity servo. The separate \texttt{voltage} input is the raw terminal voltage,
added downstream of the controller and its saturation.
