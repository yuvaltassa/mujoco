...
// kp*(qref-l) + kd*(vref-v) and the tau->V map compensates back-EMF.
TEST_F(DCMotorTest, SetpointMatchesPid) {
  ...
  while (data->time < 1.0) {
    ...
    mj_forward(model.get(), data.get());
    EXPECT_NEAR(data->actuator_force[0], data->actuator_force[1], MjTol(1e-13, 1e-6));
  }
}
...
TEST_F(DCMotorTest, StatefulVelocityMode) {
  ...
  // position error = 4.0; since x_I == Imax (2.0) and error > 0, act_dot clamps to 0
