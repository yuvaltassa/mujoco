...
// A log handler that transfers control out of a call skips its mjLEAVE, and the
// mj_freeStack of every stage it unwinds: the call is abandoned and its mjData
// needs mj_resetData before it is used again, which is what an intercepted
// error has always meant. What must survive is everything else on the thread.
jmp_buf g_escape;
...
TEST_F(OnWarnTest, AbandonedCallNeedsAResetAndLeavesTheRestAlone) {
  ...
  // the abandoned call left its stack frame behind as it always has, and the
  // data is marked as still running: mj_resetData is what clears both
  EXPECT_EQ(data->nested, 1);
...
// a call that catches an error from a nested call on another data keeps its own
// warning: the nesting of the enclosing call is untouched
