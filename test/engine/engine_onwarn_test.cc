...
// status of what ran before it, and under the stop policy does not reach it
TEST_F(OnWarnTest, IpcStepReportsStatus) {
  static constexpr char xml[] = R"(
  <mujoco>
  ...
  // auto: the bad control is zeroed and reported, the IPC step runs
  data->ctrl[0] = kNaN;
  ...
  // stop: the step ends at the actuation stage, before the integrator
  time = data->time;
  model->opt.onwarn = mjONWARN_STOP;
  EXPECT_EQ(mj_step(model.get(), data.get()), mjSTATUS_BADCTRL);
