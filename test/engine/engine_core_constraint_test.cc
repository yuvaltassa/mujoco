...
static const char* kAdhesionCeiling = R"(
<mujoco>
  <worldbody>
...
TEST_F(CoreConstraintTest, AdhesionPulloff) {
  char error[1024];
  MjModelPtr model = LoadModelFromString(kAdhesionCeiling, error, sizeof(error));
  ASSERT_THAT(model.get(), testing::NotNull()) << error;
  MjDataPtr data = MakeData(model);
...
// under tension the box hangs at positive separation which grows with load (tether)
...
TEST_F(CoreConstraintTest, AdhesionCapture) {
  std::string xml = kAdhesionCeiling;
...
TEST_F(CoreConstraintTest, AdhesionRestingPenetration) {
  ...
  for (int k=0; k < 2; k++) {
    ...
    size_t pos = xml_copy.find("ADH");
    xml_copy.replace(pos, 3, k ? "50" : "0");
    MjModelPtr model = LoadModelFromString(xml_copy.c_str(), error, sizeof(error));
    ASSERT_THAT(model.get(), testing::NotNull()) << error;
    MjDataPtr data = MakeData(model);
    while (data->time < 3) {
      mj_step(model.get(), data.get());
  ...
  EXPECT_NEAR(depth[0], depth[1], 1e-9);
}
