...
class DocTest(googletest.TestCase):
  ...
  def test_mjdata_xmacro_fields(self):
    ...
    with open(_get_path('include', 'mujoco', 'mjdata.h'), encoding='utf-8') as f:
      header = f.read()
    with open(_get_path('include', 'mujoco', 'mjxmacro.h'), encoding='utf-8') as f:
      xmacro = f.read()

    # fields of struct mjData_, one declaration per line
    body = re.search(r'typedef struct mjData_ \{(.*?)\n\} mjData;', header, re.S).group(1)
    fields = []
    ...
    xmacro_fields = set()
    for macro in re.finditer(r'#define MJDATA_\w*((?:[^\n]*\\\n)*[^\n]*)', xmacro):
    ...
    excluded = {'buffer', 'arena', 'signature'}
    missing = [f for f in fields if f not in xmacro_fields and f not in excluded]
