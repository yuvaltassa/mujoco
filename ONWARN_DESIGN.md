...
*Design, 2026-09-11; landing plan revised 2026-09-20. Branch `mjok-returns`, three commits: the
status returns; the `onwarn` option, with the `autoreset` flag deprecated but still honored; and
the removal of the flag, which can land a release later. The status return is that of the 2026-08
proposal on branch `onwarn`, narrowed to the functions that can report one. Branch `mjok` holds
the field-only alternative it was compared against (see *Why a return value* below). A separate
...
3. **`mjDSBL_AUTORESET` is deprecated, then removed**, subsumed by the attribute: flag enabled
   (default) ≡ `auto`, flag disabled ≡ `continue`. While deprecated it is still honored: disabled,
   it withholds the resets of `auto`, and it has no effect under `continue` and `stop`.
...
without rolling back (inside the collision stage the return is immediate, at the call that warned:
with the arena exhausted by the contacts accumulated so far, the next scratch allocation would be
a fatal error): what ran before the warning is in `mjData`, what would have run after it is not,
and the stage that warned may have run partway (the integrator advances the activations before the
sleep re-forward that can warn; RK4 restores the state and the time but leaves the
...
re-forward after islands fall asleep, in the pre-step work shared by `mj_advance` and `mj_commit`,
is a stage of the step like any other. The experimental IPC integrator `mj_ipc` keeps the status
of its inner constraint solves and of its commit, and under `stop` ends at the solve that warned,
with the position-dependent fields back at the pre-step configuration and its frames released.
...
## Deprecating, then removing `mjDSBL_AUTORESET`
...
- Deprecation, in the commit that adds the option: the flag keeps its bit, its schema attribute
  and its reader and writer mapping, so nothing is renumbered and no model stops loading. The
  engine consults it in one place, `mji_autoreset(m)`, true under `auto` with the flag enabled,
  which replaces the flag test in the three checks. The flag can therefore only withhold the
  resets of `auto`, never add one, and is ignored under `continue` and `stop`. It is not mapped
  onto the option, so there is no precedence question between an explicit `onwarn` and the
  flag, which was the wart of the grace-period mapping an earlier draft rejected. XML that
  specifies the attribute prints a deprecation warning; setting the bit programmatically is
  silent. simulate, Studio and the USD exporter treat `auto` with the flag disabled as
  `continue`.
- Removal, a commit of its own that can land a release later: `mjDSBL_AUTORESET = 1<<16` is
  deleted from `mjtDisableBit`; the three later bits (`NATIVECCD`, `ISLAND`, `MULTICCD`)
  renumber and `mjNDISABLE` drops to 19 (precedent: earlier flag removals renumbered).
  Everything indexed by flag order is swept: the `mjcf.schema` keyword table, simulate's
  flag-name UI arrays, the USD exporter, Studio's `step_control.cc`. Checked-in generated files
  that carry the enum (the Python introspect tables, the WASM bindings) are regenerated; the
  Unity C# bindings are generated by tooling that does not live in this repository and are
  regenerated at import. `<flag autoreset="disable"/>` becomes a schema error, and the changelog
  carries the one-line replacement (`onwarn="continue"`).
- The USD schema is the exception: its `mjc:flag:autoreset` attribute is published separately,
  so the decoder maps a disabled flag to `onwarn=continue` rather than dropping it, from the
  commit that adds the option. Replacing that attribute with an onwarn token belongs to the USD
  schema owners.
