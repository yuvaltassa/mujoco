...
// Benchmark and parity harness for scene updates through the bridge.
//
...
// rendering parity.
//
// Usage: scene_benchmark <model.xml> <nframes> [--retained] [dump_prefix]
//
// With --retained, the scene is made in retained mode and updated with
// mjv_syncScene; the bridge applies the change list instead of rebuilding
// every renderable.
...
int main(int argc, char** argv) {
  ...
  for (int frame = 0; frame < nframes; ++frame) {
    ...
    if (retained) {
      mjv_syncScene(model, data, &vopt, nullptr, &cam, mjCAT_ALL, &scene);
    } else {
  ...
  std::printf("model: %s  frames: %d  ngeom: %d  mode: %s\n", argv[1], nframes,
              ngeom, retained ? "retained" : "immediate");
  if (retained) {
    std::printf("steady-state changed entries/sync: %.1f\n", changed_total / n);
  }
