...
// predict whether folding is faster than rank-1 updates for the current cone
// set, comparing estimated flop counts:  update path  U = sum over cone
// contacts of dim * pathcost of the contact's last dof (a rank-1 update walks
// the reverse-etree path of its last dof; lower dofs merge into it
// immediately);  folded path  Lflops/2 for the factorization plus S for
// J'*D*J over the D != 0 rows. Fold when 10 U > 3 (Lflops + 2 S): an update
// flop costs about twice a factorization flop (scattered Givens rotations vs
// a dense-accumulator sweep); the measured optimum constant, flat to within
// 1% over a +-30% range.
