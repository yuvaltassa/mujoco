...
void mjd_actuator_vel(const mjModel* m, mjData* d) {
  ...
  for (int i=0; i < nactuator; i++) {
    ...
    // DC motor controller damping and LuGre micro-damping
      ...
      // controller velocity derivative dV/dω: torque-space kd through the τ→V map,
      // plus the back-EMF compensation K, which cancels the bias-block term so the
      // net damping of an unclipped torque-mode motor is -kd; Vmax clipping is
      // ignored here, matching the treatment of the other saturations
      mjtNum dVdw = 0;
