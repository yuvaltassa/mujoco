...
Light::Light(filament::Engine* engine, const mjrfLightParams& params)
  ...
  if (type == filament::LightManager::Type::SPOT) {
    // Equal inner and outer angles: the light delivers its full intensity
    // everywhere inside the cone, so illuminance follows E = I/d^2
    // independent of the cutoff. An inner angle of 0 would make the whole
    // beam penumbra, dimming narrow spots by several times.
    const float outer = params.spot_cone_angle * std::numbers::pi / 180.0f;
    builder.spotLightCone(outer, outer);
  }
